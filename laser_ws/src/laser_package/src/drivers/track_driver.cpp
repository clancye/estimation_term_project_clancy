...
void Tracker::updateSpeed(float some_x, float some_y, double time_of_measurement)
{
	...
	for(int i = MEASUREMENT_MEMORY-1;i>0;i--)
	{
		...
		accel_x_temp_sum += MEASUREMENT_COEFFICIENT*accel_x_memory[i];
		accel_y_temp_sum += MEASUREMENT_COEFFICIENT*accel_y_memory[i];
	...
	current_x_accel = accel_x_temp_sum/ACCEL_MEASUREMENT_MEMORY;
	current_y_accel = accel_y_temp_sum/ACCEL_MEASUREMENT_MEMORY;
